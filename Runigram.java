...
public class Runigram {
		  ...
		  int numRows = image.length;
   	     ...
   	     for (int i = 0; i < numRows; i++) {
              for (int j = 0; j < numCols; j++) {
            	...
            	print(image[i][j]);     // prints (r,g,b)
         	    System.out.print("  "); // space between pixels
        }
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
    	...
    	for (int i = 0; i < height; i++) {
            	...
            	int sourceRow = i * originalHeight / height;
            	int sourceCol = j * originalWidth / width;
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		 //Make sure target has the same dimensions as source
    ...
    // Morph: i = 0..n, alpha = (n - i) / n
    for (int i = 0; i <= n; i++) {
        double alpha = (double) (n - i) / n;           // 1->0
        Color[][] step = blend(source, target, alpha); // blend images
